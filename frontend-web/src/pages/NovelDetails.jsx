...
const NovelDetails = () => {
  ...
  // Fallback cover if not provided
  // Fallback cover if not provided
      ...
      <div className="relative h-[60vh] min-h-[500px]">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden">
          <img 
            src={coverUrl} 
            alt="Background" 
            className="w-full h-full object-cover blur-xl scale-110 opacity-40 dark:opacity-20 bg-slate-200 dark:bg-slate-700"
            onError={(e) => {
          ...
          <div className="flex flex-col md:flex-row items-center md:items-end gap-8">
            ...
            >
              <img 
                src={coverUrl} 
                alt={novel.title} 
                className="w-full h-[280px] md:h-[380px] object-cover rounded-2xl shadow-2xl border-4 border-white dark:border-slate-800 bg-slate-100 dark:bg-slate-800"
                onError={(e) => {
                  e.target.onerror = null;
            ...
            >
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4 mb-6 text-sm text-slate-700 dark:text-slate-300">
                <div className="flex items-center bg-white/50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg backdrop-blur-md">
                  ...
                  <span className="font-semibold">{novel.rating || '0.0'}</span>
                </div>
                <div className="flex items-center bg-white/50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg backdrop-blur-md">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                <Link 
                  ...
                  className={`px-8 py-3.5 bg-primary-600 hover:bg-primary-500 text-white font-bold rounded-xl shadow-lg shadow-primary-500/30 transition-all flex items-center justify-center gap-2 transform hover:-translate-y-1 ${chapters.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'synopsis' && (
                ...
                >
                  ...
                  <div className="prose dark:prose-invert max-w-none text-slate-700 dark:text-slate-300 text-lg leading-relaxed whitespace-pre-wrap">
              ...
              {activeTab === 'chapters' && (
                ...
                >
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <span className="text-sm text-slate-500 dark:text-slate-400">Hiển thị {chapters.length} chương</span>
                  </div>
                  
                  {chapters.length === 0 ? (
                    <div className="text-center text-slate-500 py-8">Chưa có chương nào được đăng.</div>
                  ) : (
